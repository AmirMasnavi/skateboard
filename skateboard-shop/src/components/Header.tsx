...
function Header() {
  ...
  const toggleMobileMenu = () => {
    setIsMobileMenuOpen(!isMobileMenuOpen); // This toggles the state between true and false
  };

  return (
    <> {/* We use a Fragment (<>) here to return multiple elements */}
      <header className="bg-[#121212] text-[#F5F5F5] p-4 fixed top-0 left-0 w-full z-50 border-b border-[#333333]">
        <div className="container mx-auto flex justify-between items-center">
          ...
          <div className="flex items-center space-x-4">
            <button className="hover:text-[#FCEE09] transition-colors">
              <Search size={20} />
            </button>
            <button className="hover:text-[#FCEE09] transition-colors">
              <Sun size={20} />
            ...
            <button
              ...
              onClick={toggleMobileMenu} // Add the click handler
            >
              {isMobileMenuOpen ? <X size={24} /> : <Menu size={24} />} {/* Change icon based on state */}
            </button>
          </div>
        </div>
      </header>
      ...
      {isMobileMenuOpen && (
        <div className="md:hidden fixed inset-0 bg-[#121212] z-40 flex items-center justify-center">
          <nav>
